...
<<<<<<< HEAD
    ...
    try {
      // ── Primary: GPT match against real WhatsApp demand pool ──────────
      const backendRes = await fetch('/api/match', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        signal: AbortSignal.timeout(60000), // GPT scoring can take ~30s for many demands
        body: JSON.stringify({ asset: {
          ...
          bathrooms:  parseInt(asset.bathrooms) || 0,
          area_sqm:   parseInt(asset.area) || 0,
          finishing:  asset.finishing,
          compound:   asset.compound,
          floor:      asset.floor,
          view:       asset.view,
      ...
      if (backendRes.ok) {
        ...
        if (ct.includes('json')) {
          ...
          const poolSize = data.demandPoolSize || 0
          setDemandPoolSize(poolSize)

          // Map backend GPT matches to MatchedDemand shape
          const gptMatches: MatchedDemand[] = (data.matches || []).map((m: any, i: number) => {
            const ex = m.extracted || {}
            const partial = {
              budget_min:  ex.budget_min  || parseInt(asset.price) * 0.75,
              budget_max:  ex.budget_max  || ex.price || parseInt(asset.price) * 1.25,
              bedrooms:    ex.bedrooms    || parseInt(asset.bedrooms) || 0,
              bathrooms:   ex.bathrooms   || parseInt(asset.bathrooms) || 2,
              type:        ex.property_type || asset.type,
              location:    ex.location    || asset.location,
              area_min:    ex.area_sqm ? Math.round(ex.area_sqm * 0.85) : parseInt(asset.area) * 0.75,
              area_max:    ex.area_sqm ? Math.round(ex.area_sqm * 1.15) : parseInt(asset.area) * 1.25,
              purpose:     ex.purpose     || asset.purpose,
              finishing:   ex.finishing   || 'any',
            }
            // Use GPT breakdown if available, else calculate
            const breakdown = m.breakdown
              ? {
                  location:  m.breakdown.location  || 0,
                  price:     m.breakdown.price      || 0,
                  type:      m.breakdown.type        || 0,
                  bedrooms:  m.breakdown.size        || m.breakdown.bedrooms || 0,
                  area:      m.breakdown.size        || m.breakdown.area     || 0,
                  finishing: m.breakdown.finishing   || 0,
                }
              : calcScore(asset, partial)

            return {
              id:          i + 1,
              buyer_name:  m.sender || `Buyer ${i+1}`,
              buyer_phone: m.phone  || '',
              score:       (m.score || 0) / 100,
              source:      'WhatsApp' as Platform,
              notes:       m.notes  || m.message?.slice(0, 100) || '',
              created_at:  new Date(m.timestamp * 1000).toISOString(),
              contact_time: 'Via WhatsApp',
          ...
          if (gptMatches.length > 0) {
            const sorted = gptMatches.sort((a, b) => b.score - a.score)
            setMatches(sorted)
            setMatchSource('gpt')
            setHasSearched(true)
          ...
          // GPT returned empty (no demand pool) → note and fall through to mock
          if (poolSize === 0) {
            console.info('[AssetMatcher] WA demand pool is empty — falling back to illustrative mock data')
          }
        }
      }
    } catch (err) {
      console.warn('[AssetMatcher] Backend match failed:', err)
    }

    // ── Fallback: illustrative mock data (shown when backend is down or demand pool empty) ──
...
<<<<<<< HEAD
      ...
      {hasSearched && matchSource && (
        <div style={{
          ...
          background: matchSource === 'gpt' ? 'rgba(167,139,250,0.08)' : 'rgba(245,158,11,0.08)',
          border: `1px solid ${matchSource === 'gpt' ? 'rgba(167,139,250,0.3)' : 'rgba(245,158,11,0.3)'}`,
          color: matchSource === 'gpt' ? '#a78bfa' : '#f59e0b',
          display: 'flex', alignItems: 'center', gap: 10,
        }}>
          {matchSource === 'gpt' ? (
            <>
              <span>⚡</span>
              <span><strong>GPT-5-mini Scoring</strong> — Matched against {demandPoolSize} real WhatsApp demand messages. Scores are AI-evaluated per property match.</span>
            </>
          ) : (
            <>
              <span>📊</span>
              <span><strong>Illustrative Demo Data</strong> — No real WA demand messages yet (pool empty or backend offline). Configure credentials in ⚙️ Settings → Apply to Server to see real buyer matches.</span>
            </>
          )}
        </div>
      )}
      ...
      {hasSearched && (
        <div className="grid grid-cols-4 stagger" style={{ gap: 14 }}>
          ...
          <StatCard
            title={matchSource === 'gpt' ? 'WA Demand Pool' : 'Platforms'}
            value={matchSource === 'gpt' ? demandPoolSize : filterPlatforms.size}
            icon={matchSource === 'gpt' ? '💬' : '🌐'}
