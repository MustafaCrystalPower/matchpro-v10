...
=======
      {/* Weekly Activity Heat Grid + Gauges */}
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '20px' }}>
        <Card title="Weekly Activity Pattern" subtitle="Peak engagement hours across the market week">
          <WeeklyHeatGrid />
        </Card>
        <Card title="System Performance" subtitle="Intelligence quality indicators">
...
function WeeklyHeatGrid() {
  ...
  const grid  = days.map(d => hours.map(h => {
    // Simulate activity — weekdays peak mid-day, weekends peak morning/evening
    const isWeekend  = d === 'Sat' || d === 'Sun'
    const isPeakHour = ['1pm','3pm'].includes(h)
    const base = isWeekend ? 30 : isPeakHour ? 75 : 45
    return Math.min(100, Math.max(5, base + Math.floor(Math.random() * 35 - 10)))
  }))

  return (
    <div>
      {/* Hour labels */}
      <div style={{ display: 'flex', marginBottom: '6px', paddingLeft: '36px', gap: '3px' }}>
        {hours.map(h => (
          <div key={h} style={{ flex: 1, textAlign: 'center', fontSize: '0.6rem', color: 'var(--text-muted)', fontWeight: 600 }}>{h}</div>
        ))}
      </div>
      {/* Grid rows */}
      {days.map((day, di) => (
        <div key={day} style={{ display: 'flex', alignItems: 'center', gap: '3px', marginBottom: '3px' }}>
          <div style={{ width: 32, fontSize: '0.68rem', color: 'var(--text-muted)', fontWeight: 700, flexShrink: 0 }}>{day}</div>
          {grid[di].map((val, hi) => {
      ...
      <div style={{ display: 'flex', gap: '14px', marginTop: '10px', justifyContent: 'flex-end' }}>
        {[['rgba(14,165,233,0.4)','Low'],['rgba(245,158,11,0.55)','Mid'],['rgba(239,68,68,0.75)','High']].map(([c, l]) => (
          <div key={l as string} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            <div style={{ width: 12, height: 12, borderRadius: '3px', background: c as string }} />
            <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>{l as string}</span>
          </div>
        ))}
      </div>
>>>>>>> origin/main
